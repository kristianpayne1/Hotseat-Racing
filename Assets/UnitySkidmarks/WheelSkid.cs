...
public class WheelSkid : MonoBehaviour {
	...
	[SerializeField]
	AudioSource skidStart;
	[SerializeField]
	AudioSource skidding;
	...
	protected void LateUpdate() {
		if (wheelCollider.GetGroundHit(out wheelHitInfo))
		{
			...
			if (skidTotal >= SKID_FX_SPEED) {
				...
				skidStart.volume = maxVolume * intensity;
				skidding.volume = maxVolume * intensity;
				if (!isSkidding) isSkidding = true;
				if (!skidding.isPlaying) skidding.Play();

				// Account for further movement since the last FixedUpdate
				Vector3 skidPoint = wheelHitInfo.point + (rb.velocity * (Time.time - lastFixedUpdateTime)) * 4;
				lastSkid = skidmarksController.AddSkidMark(skidPoint, wheelHitInfo.normal, intensity, lastSkid);
			}
			else {
				if (isSkidding) { skidding.Stop(); skidStart.Stop(); isSkidding = false; }
